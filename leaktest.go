...
func interestingGoroutine(g string, fn FilterFunc) (*goroutine, error) {
	sl := strings.SplitN(g, "\n", 2)
	if len(sl) != 2 {
...
func interestingGoroutines(t ErrorReporter, fn FilterFunc) []*goroutine {
	buf := make([]byte, 2<<20)
	buf = buf[:runtime.Stack(buf, true)]
	var gs []*goroutine
	for _, g := range strings.Split(string(buf), "\n\n") {
		gr, err := interestingGoroutine(g, fn)
		if err != nil {
...
func Check(t ErrorReporter, fn FilterFunc) func() {
	return CheckTimeout(t, 5*time.Second, fn)
}

// CheckTimeout is the same as Check, but with a configurable timeout
func CheckTimeout(t ErrorReporter, dur time.Duration, ffn FilterFunc) func() {
	ctx, cancel := context.WithCancel(context.Background())
	fn := CheckContext(ctx, t, ffn)
	return func() {
...
func CheckContext(ctx context.Context, t ErrorReporter, fn FilterFunc) func() {
	orig := map[uint64]bool{}
	for _, g := range interestingGoroutines(t, fn) {
		orig[g.id] = true
	}
	return func() {
		var leaked []string
